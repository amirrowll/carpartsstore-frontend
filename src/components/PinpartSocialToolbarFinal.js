...
const PinpartSocialToolbarFinal = () => {
    ...
    const handleChannelClick = (channelId, url) => {
    ...
    return (<>
      ...
      <div className="fixed left-4 bottom-4 z-[9999]">
        <button onClick={() => setIsOpen(!isOpen)} className="group flex items-center justify-center w-16 h-16 bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-full shadow-2xl hover:shadow-3xl hover:scale-110 transition-all duration-300 relative overflow-hidden" aria-label="کانال های ارتباطی پین پارت">
          ...
          <div className="relative z-10">
            {isOpen ? (<Close className="!w-7 !h-7"/>) : (<div className="flex flex-col items-center">
        ...
        {/* Tooltip when closed */}
        {!isOpen && (<div className="absolute left-20 bottom-4 bg-gradient-to-r from-gray-900 to-gray-800 text-white text-sm px-4 py-2 rounded-lg shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap pointer-events-none">
      ...
      {/* Expanded Toolbar */}
      {isOpen && (<div className="fixed left-4 bottom-24 z-[9998] animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden w-80 max-h-[80vh] overflow-y-auto">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-4 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={handleClose} className="p-1.5 text-white/80 hover:text-white hover:bg-white/20 rounded-lg transition-colors" aria-label="بستن">
            ...
            <div className="p-4">
              <div className="space-y-3">
                {socialChannels.map((channel) => (<button key={channel.id} onClick={() => handleChannelClick(channel.id, channel.url)} className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] ${activeChannel === channel.id
      ...
      {/* Backdrop */}
      {isOpen && (<div className="fixed inset-0 bg-black/20 z-[9997] backdrop-blur-sm" onClick={handleClose} aria-hidden="true"/>)}
