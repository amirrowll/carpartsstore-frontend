...
const StoriesSection = ({ position = 'top', maxStories = 8, showAddButton = false, onAddStory }) => {
    ...
    return (<div className={`w-full ${position === 'top' ? 'pt-4 mb-4' : 'mt-4'}`}>
      <div className="container mx-auto px-4">
        ...
        <div className="bg-transparent">
            {loading ? (<div className="flex justify-center items-center py-4">
              ...
              </div>) : error ? (<div className="text-center py-4 text-red-600 text-sm">
                {error}
              </div>) : stories.length > 0 ? (<div className="flex gap-4 overflow-x-auto pb-2 px-1 scrollbar-hide">
                {stories.map((story) => (<StoryCircle key={story.id} story={story} onClick={handleStoryClick} size="md"/>))}
              </div>) : null}
          </div>) : (
        ...
        <div className="bg-gradient-to-r from-gray-50 to-white rounded-2xl border border-gray-200 shadow-lg p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={handleAddStory} className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-4 py-2 rounded-xl font-medium transition-all hover:scale-105">
                <Add />
            ...
            {loading ? (<div className="flex justify-center items-center py-12">
              ...
              </div>) : error ? (<div className="text-center py-8 text-red-600 bg-red-50 rounded-xl">
                {error}
                ...
                <button onClick={handleAddStory} className="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-3 rounded-xl font-medium transition-all hover:scale-105">
                  <Add />
                  <span>ایجاد اولین استوری</span>
                </button>
              </div>) : (<div className="flex gap-6 overflow-x-auto pb-4 px-2 scrollbar-hide">
                {stories.map((story) => (<StoryCircle key={story.id} story={story} onClick={handleStoryClick} size="md"/>))}
                
                {/* Add story button at the end */}
                <div className="flex flex-col items-center">
                  <button onClick={handleAddStory} className="w-20 h-20 rounded-full border-2 border-dashed border-gray-300 hover:border-blue-500 hover:bg-blue-50 flex items-center justify-center transition-all group">
            ...
            {/* Stats */}
            {stories.length > 0 && (<div className="mt-6 pt-6 border-t border-gray-200">
        ...
        {/* Story Viewer Modal */}
        {showViewer && selectedStoryIndex !== null && (<StoryViewer stories={stories} initialIndex={selectedStoryIndex} onClose={handleCloseViewer}/>)}
      </div>
    </div>);
};
