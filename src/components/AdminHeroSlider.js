...
const AdminHeroSlider = () => {
    ...
    const toggleSlideStatus = async (id, currentStatus) => {
        try {
            ...
            // Update local state
            setSlides(prev => prev.map(slide => slide.id === id ? { ...slide, isActive: !currentStatus } : slide));
    ...
    const activeSlides = slides.filter(slide => slide.isActive);
    return (<div className="bg-white rounded-2xl shadow-xl p-6 space-y-8">
      {/* Messages */}
      {error && (<div className="rounded-xl bg-red-50 border border-red-200 p-4">
          <p className="text-red-700 font-medium">{error}</p>
        </div>)}
      
      {successMessage && (<div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4">
      ...
      <div className="space-y-4">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900 min-h-[400px]">
          {loading ? (<div className="h-full flex items-center justify-center">
            ...
            </div>) : activeSlides.length > 0 ? (<>
              {/* Slides */}
              <div className="relative h-full">
                {activeSlides.map((slide, index) => (<div key={slide.id} className={`absolute inset-0 transition-all duration-1000 ease-in-out ${index === currentSlide
              ...
              <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex items-center gap-4 bg-black/30 backdrop-blur-sm rounded-full px-4 py-2">
                ...
                <div className="flex items-center gap-2">
                  {activeSlides.map((_, index) => (<button key={index} onClick={() => goToSlide(index)} className={`w-2 h-2 rounded-full transition-all ${index === currentSlide
                    ? 'bg-white w-6'
        ...
        <div className="flex items-center gap-4 bg-gray-50 rounded-xl p-4">
          <div className="flex-1">
            ...
            <input type="range" min="1000" max="10000" step="1000" value={slideInterval} onChange={(e) => setSlideInterval(parseInt(e.target.value))} className="w-full"/>
          ...
          <div className="flex gap-2">
            <button onClick={saveSlideOrder} disabled={saving} className="px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
      ...
      <div className="space-y-6">
        ...
        {uploadedFiles.length > 0 && (<div className="bg-gray-50 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              {uploadedFiles.map((file, index) => (<div key={index} className="relative group">
                  ...
                  <button onClick={() => removeUploadedFile(index)} className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
                    <X className="h-4 w-4"/>
                  </button>
            ...
            <div className="space-y-4 mb-4">
              <div>
                ...
                <input type="text" value={newSlideTitle} onChange={(e) => setNewSlideTitle(e.target.value)} placeholder="مثال: قطعات خودرو با کیفیت" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"/>
              </div>
              ...
              <div>
                ...
                <textarea value={newSlideDescription} onChange={(e) => setNewSlideDescription(e.target.value)} placeholder="مثال: بزرگترین مجموعه قطعات یدکی خودرو با گارانتی کیفیت و قیمت مناسب" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" rows={2}/>
              </div>
            </div>

            <button onClick={addNewSlide} disabled={saving || uploadedFiles.length === 0} className="w-full px-6 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white font-medium rounded-lg hover:from-emerald-700 hover:to-emerald-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {saving ? (<Loader className="h-5 w-5 animate-spin"/>) : (<Upload className="h-5 w-5"/>)}
        ...
        <div className="space-y-3">
          ...
          {loading ? (<div className="text-center py-8 text-gray-500">
              ...
              در حال بارگذاری اسلایدها...
            </div>) : slides.length > 0 ? (slides.map((slide) => (<div key={slide.id} className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${slide.isActive ? 'border-gray-200 hover:bg-gray-50' : 'border-gray-100 bg-gray-50 opacity-60'}`}>
                ...
                target.src = 'https://placehold.co/100x50/1e3a8a/ffffff?text=Car'; // تصویر جایگزین کوچک
                    ...
                    {!slide.isActive && (<div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {slide.isActive ? (<span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">
                        فعال
                      </span>) : (<span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">
                        غیرفعال
                      </span>)}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => toggleSlideStatus(slide.id, slide.isActive)} disabled={saving} className={`p-2 rounded-lg ${slide.isActive
                ? 'text-emerald-600 hover:bg-emerald-50'
                : 'text-gray-600 hover:bg-gray-100'}`}>
                    {slide.isActive ? <EyeOff className="h-4 w-4"/> : <Eye className="h-4 w-4"/>}
                  </button>
                  <button onClick={() => removeSlide(slide.id)} disabled={saving} className="p-2 text-red-500 hover:bg-red-50 rounded-lg disabled:opacity-50">
                    <X className="h-4 w-4"/>
                  </button>
                </div>
