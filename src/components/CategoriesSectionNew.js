...
const CategoriesSectionNew = () => {
    ...
    if (loading) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (<div key={i} className="bg-white rounded-3xl shadow-xl p-8 animate-pulse">
    ...
    return (<section className="py-20 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6">
            دسته بندی <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500">محصولات</span>
        ...
        <div className="relative pb-24">
          <Swiper modules={[Navigation, Pagination, Autoplay]} spaceBetween={30} slidesPerView={1} navigation={{
            nextEl: '.custom-next',
            prevEl: '.custom-prev',
        }} pagination={{
            ...
            return (<SwiperSlide key={category.id}>
                  <div className="relative bg-white rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden group h-full">
                    ...
                    <div className="relative h-48 overflow-hidden rounded-t-3xl">
                      ...
                      {category.imageUrl ? (<img src={completeImageUrl(category.imageUrl)} alt={category.name} className="w-full h-full object-cover" onError={(e) => {
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                        <Link to={`/category/${category.id}/${encodeURIComponent(category.name.replace(/\s+/g, '-'))}`} className="text-blue-600 font-bold hover:text-blue-700 flex items-center gap-2 transition-all text-sm">
                          مشاهده محصولات
          ...
          <div className="absolute bottom-4 left-0 right-0 z-10 flex items-center justify-center gap-8 mt-12">
            {/* Previous Button */}
            <button className="custom-prev w-10 h-10 bg-white rounded-full shadow-lg hover:shadow-xl border border-gray-200 hover:border-blue-500 transition-all flex items-center justify-center" aria-label="قبلی">
            ...
            {/* Next Button */}
            <button className="custom-next w-10 h-10 bg-white rounded-full shadow-lg hover:shadow-xl border border-gray-200 hover:border-blue-500 transition-all flex items-center justify-center" aria-label="بعدی">
              <svg className="w-5 h-5 text-gray-700 hover:text-blue-600 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24">
