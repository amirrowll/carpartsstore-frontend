...
import { Dashboard, Inventory, Category, People, Receipt, Settings, Menu, ChevronLeft, ChevronRight, AddPhotoAlternate, Logout, Notifications, TrendingUp, Store, Brightness4, Language } from '@mui/icons-material';
        ...
        <div className="h-full px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden md:flex items-center gap-2">
              {quickActions.slice(0, 2).map((action, index) => (<button key={index} className={`flex items-center gap-2 ${action.color} text-white px-4 py-2 rounded-xl hover:opacity-90 transition-opacity`}>
                  {action.icon}
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto py-6">
            ...
            return (<div key={index}>
                    <Link to={item.path} onClick={() => setActiveMenu(activeMenu === item.title ? '' : item.title)} className={`
                    ...
                    : 'text-gray-700 hover:bg-gray-100 hover:text-gray-900'}
                      ...
                      {sidebarOpen && (<>
                          ...
                          {item.subItems && (<ChevronLeft className={`text-sm transition-transform ${activeMenu === item.title ? 'rotate-90' : ''}`}/>)}
                        </>)}
                    </Link>

                    {/* Sub items */}
                    {sidebarOpen && item.subItems && activeMenu === item.title && (<div className="mt-1 ml-8 space-y-1">
                        {item.subItems.map((subItem, subIndex) => (<Link key={subIndex} to={subItem.path} className={`
