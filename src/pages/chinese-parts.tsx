...
import { LocalShipping, EmojiEvents, Phone } from '@mui/icons-material';
...
const ChinesePartsPage: React.FC = () => {
  const [products, setProducts] = useState<Product[]>(sampleProducts.filter((p) => p.brand === 'Chinese'));

  useEffect(() => {
    productApi
      .getProductsByBrand('Chinese', 24)
      .then((data) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4 py-10">
        <section className="rounded-[32px] bg-gradient-to-r from-cyan-500 to-sky-600 p-8 md:p-10 text-white shadow-2xl mb-10">
          <h1 className="text-3xl md:text-4xl font-extrabold">لوازم چینی</h1>
          <p className="mt-4 max-w-2xl text-base md:text-lg text-cyan-100/90">
            بهترین قطعات اقتصادی و باکیفیت را برای خودروهای چینی پیدا کنید. تجربه بهتر، نصب آسانتر و تضمین کیفیت بالاتر.
          </p>
          <div className="mt-8 flex justify-center gap-4">
            <div className="rounded-2xl md:rounded-3xl bg-white/10 p-4 md:p-5 text-center flex flex-col items-center justify-center">
              <LocalShipping className="text-2xl mb-2 text-white" />
              <h3 className="font-bold">سریعترین ارسال</h3>
            </div>
            <div className="rounded-2xl md:rounded-3xl bg-white/10 p-4 md:p-5 text-center flex flex-col items-center justify-center">
              <EmojiEvents className="text-2xl mb-2 text-white" />
              <h3 className="font-bold">گارانتی اصالت</h3>
            </div>
            <div className="rounded-2xl md:rounded-3xl bg-white/10 p-4 md:p-5 text-center flex flex-col items-center justify-center">
              <Phone className="text-2xl mb-2 text-white" />
              <h3 className="font-bold">پشتیبانی 24/7</h3>
            </div>
          </div>
        </section>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {products.map((product) => (
            <div key={product.id} className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-lg transition hover:-translate-y-1">
              <div className="relative h-48 overflow-hidden rounded-3xl bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center">
                <img 
                  ...
                  alt={product.name} 
                  className="h-40 w-40 object-contain p-4"
                  onError={(e) => {
...
export default ChinesePartsPage;