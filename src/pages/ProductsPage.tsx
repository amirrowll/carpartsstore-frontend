      ...
      <div className="container mx-auto px-4 py-10">
        <div className="rounded-[32px] bg-white p-8 shadow-xl">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="animate-pulse rounded-[28px] border border-slate-200 bg-slate-50 p-6">
    ...
    <div className="container mx-auto px-4 py-10">
      <div className="rounded-[32px] bg-white p-8 shadow-xl">
        ...
        <div className="mb-8">
          <div className="flex flex-wrap gap-3 justify-center">
            <button
              ...
              className={`px-5 py-2.5 rounded-xl font-medium transition-all ${activeBrandFilter === 'all' ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:border-blue-400 hover:shadow-md'}`}
            >
              همه محصولات
            </button>
            <button
              onClick={() => setActiveBrandFilter('Saipa')}
              className={`px-5 py-2.5 rounded-xl font-medium transition-all ${activeBrandFilter === 'Saipa' ? 'bg-gradient-to-r from-orange-500 to-red-500 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:border-orange-400 hover:shadow-md'}`}
            >
              لوازم سایپا
            </button>
            <button
              onClick={() => setActiveBrandFilter('IranKhodro')}
              className={`px-5 py-2.5 rounded-xl font-medium transition-all ${activeBrandFilter === 'IranKhodro' ? 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:border-indigo-400 hover:shadow-md'}`}
            >
              لوازم ایران خودرو
            </button>
            <button
              onClick={() => setActiveBrandFilter('Chinese')}
              className={`px-5 py-2.5 rounded-xl font-medium transition-all ${activeBrandFilter === 'Chinese' ? 'bg-gradient-to-r from-cyan-500 to-sky-500 text-white shadow-lg' : 'bg-white text-gray-700 border border-gray-300 hover:border-cyan-400 hover:shadow-md'}`}
            >
              لوازم چینی
            </button>
          </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="mt-10 text-center py-16 bg-gray-50 rounded-2xl">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {filteredProducts.map((product) => (
              <div key={product.id} className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm transition-all hover:-translate-y-2 hover:shadow-xl">
