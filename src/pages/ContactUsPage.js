        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {contactMethods.map((method) => (<div key={method.id} className={`${method.color} rounded-2xl border border-gray-200 p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
                  <div className="flex flex-col h-full">
                    ...
                    <div className="mb-4">
                      {method.number && (<div className="bg-white/50 rounded-lg p-3 mb-2 text-right">
                          <p className="text-sm text-gray-500 mb-1">شماره تماس</p>
                          <p className="font-bold text-gray-800 text-lg">{method.number}</p>
                        </div>)}
                      {method.username && (<div className="bg-white/50 rounded-lg p-3 mb-2 text-right">
                          <p className="text-sm text-gray-500 mb-1">آیدی</p>
                          <p className="font-bold text-gray-800 text-lg">{method.username}</p>
                        </div>)}
                    </div>

                    <button onClick={method.action} className={`w-full font-extrabold py-3 px-4 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-lg hover:shadow-xl border-2 border-white/30 ${method.id === 1 ? 'bg-blue-600 text-white' : method.id === 2 ? 'bg-green-600 text-white' : method.id === 3 ? 'bg-blue-500 text-white' : 'bg-pink-600 text-white'}`}>
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl mx-auto">
              {managementContact.map((method) => (<div key={method.id} className={`${method.color} rounded-2xl border border-gray-200 p-6 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
                  <div className="flex flex-col h-full">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="p-3 bg-white rounded-xl shadow-sm">
                        {method.icon}
                    ...
                    <button onClick={method.action} className={`w-full font-extrabold py-3 px-4 rounded-xl hover:opacity-90 transition-opacity flex items-center justify-center gap-2 shadow-lg hover:shadow-xl border-2 border-white/30 ${method.id === 1 ? 'bg-purple-600 text-white' : 'bg-green-600 text-white'}`}>
                      <span className="text-lg">{method.buttonText}</span>
                      <ArrowRight className="h-5 w-5"/>
                    </button>
                  </div>
